...
import { deletePurchaseService, generateBillNumber, getAllPurchaseService, getPurchaseByIdService, getRowService, totalPurchaseQuantity } from "../services/purchase-service";
...
export const updatePurchaseController = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const { partyName, number, addDescription, itemName, qty, unit, priceUnitTax, paymentType, purchaseTax } = req.body;
        ...
        await purchase.findOneAndUpdate(
            ...
            {
                $set: {
                    partyName, number, addDescription, itemName, qty, unit, priceUnitTax, paymentType,
                    amount: taxOnAmount, purchaseTax,
                },
            },
        ).then((data) => {
            res.send(data);
...
export const addPurchaseRowController = async (req: Request, res: Response) => {
    try {
        ...
        const purchaseFind = await purchase.findOne({ _id: id }).populate("row").lean();
        if (!purchaseFind) {
            return res.status(400).json({ message: "Invalid ID provided." });
        }
...
export const updatePurchaseRowController = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        ...
        // updating row
        await purchaseRow.findOneAndUpdate(
            { _id: id },
            {
                $set: {
                    itemName, qty, unit, purchaseTax, priceUnitTax, amount: taxOnAmount,
                },
            },
        ).then((data) => {
            res.send(data);
...
export const deletePurchaseRowController = async (req: Request, res: Response) => {
    const { id } = req.params;
    try {
